...
/**
 * Get available subjects for a specific course/year/semester/department combination
 */
export const getSubjectsForContext = (
  ...
  return Object.keys(subjectsData[course]?.[year]?.[semester]?.[department] || {});
};
...
/**
 * Get semesters for a course and year (if defined)
 */
export const getSemestersForCourseYear = (
  courseData: AcademicConfig['courseData'],
  course: string,
  year: string
): string[] => {
  if (!courseData[course]) return ['Odd', 'Even'];
  
  // Use year-specific semesters if defined
  if (courseData[course].yearSemesters && courseData[course].yearSemesters![year]) {
    return courseData[course].yearSemesters![year];
  }
  
  // Fallback to course-level semesters or global default
  return courseData[course].semesters || ['Odd', 'Even'];
};

/**
 * Legacy compatibility: Get semesters for a course
 */
